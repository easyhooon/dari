...
gh api repos/{OWNER}/{REPO}/pulls/{PR_NUMBER}/comments \
  -f body="✅ https://github.com/{OWNER}/{REPO}/commit/{FULL_COMMIT_HASH} 에 반영완료!" \
...
2. 사용자가 승인하면, 각 코멘트에 대댓글로 거절 사유를 작성한다:

```bash
gh api repos/{OWNER}/{REPO}/pulls/{PR_NUMBER}/comments \
  -f body="해당 제안은 검토 결과 현재 반영하지 않았습니다.

**사유**: {구체적인 거절 사유를 명확하게 작성}" \
  -F in_reply_to={FIRST_COMMENT_DATABASE_ID}
```
