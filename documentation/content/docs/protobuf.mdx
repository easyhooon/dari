...
Use a stable `requestId` to pair a request and response. Pass `null` only for standalone or fire-and-forget messages. `displayName` is optional and is stored separately from the stable `handlerName`.
